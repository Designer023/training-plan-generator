...
import {WeekPlan, DayPlan, UserSpec, Activity, Day, Run, ProgressType, CrossType, CrossRoutineType} from "./types";
...
const weekSchedule: Record<string, string[]> = {
    "regular":  ["long", "rest_or_easy", "hit", "rest_or_easy", "hit", "base", "rest"],
    "double":   ["pair_2", "rest_or_easy", "hit", "rest_or_easy", "base", "rest", "pair_1"],
    "ultra":    ["long+", "rest_or_easy", "hit", "rest_or_easy", "base", "rest", "easy"],
    "recovery": ["long_r", "rest_or_easy", "hit", "base", "hit", "rest_or_easy", "parkrun"],
    "taper":    ["long_taper", "rest_or_easy", "base", "hit", "base", "rest_or_easy", "easy"],
    "race":     ["race", "rest_or_easy", "base", "rest_or_easy", "rest_or_easy", "rest", "easy"]
};
...
const generateIntervals = () => {
    const groups = [[1000, "4-6", HR.SPEED], [800, "6-8", HR.SPEED], [100, "8-10", [HR.SPEED[0], HR.SPRINT[1]]]];

    const selected = groups[Math.floor(Math.random() * groups.length)];

    return { text: `Run for ${selected[0]}m @ HR ${selected[2]}%. Repeat ${selected[1]} times` };
};
...
const hitTypes = [
    {
        category: "Fartlek",
        description: "A steady paced run but interspersed with periods of faster 'anaerobic paced' running at random times of your choice.",
        routine: [{ text: "3 minute warmup", effort: HR.WARMUP }, { text: `Run for 30 mins @ HR ${HR.STAMINA}%, with random intervals @ HR ${HR.SPEED}% ` }, { text: "3 minute cooldown", effort: HR.WARMUP }]
    },
    {
        category: "Interval",
        description: "Interval sessions are excellent at increasing your training threshold by building a tolerance to lactic acid and making the body more efficient at clearing it away. ",
        routine: [{ text: "3 minute warmup", effort: HR.WARMUP }, generateIntervals(), { text: "3 minute cooldown", effort: HR.WARMUP }]
    },
...

    {
        ...
        description: "Running up hills builds strength and power in your legs, which, as well as helping you bound up hills like a mountain goat, will transfer into faster speeds on the flat, too",
...

    {
        ...
        routine: [{ text: "3 minute warmup", effort: HR.WARMUP }, { text: "20 mins", effort: HR.ENDURANCE }, { text: "15 mins", effort: HR.STAMINA }, { text: "10 mins", effort: HR.SPEED }, { text: "3 minute cooldown", effort: HR.WARMUP }]
    },
    {
...
export const calculateDistanceForWeek = (week: number = 1, multiplier: number = 1, start: number = 1000, growthRatePercent:number=5, max:number=5000, planStrategies: string[]): number => {
    let dist = start;
...
export class Progress {
    ...
    constructor(userSpec: UserSpec, startDistance: number, endDistance: number, planLength: number, planTailLength: number, planRecoverySpacing: number = 4) {
        ...
        const growthWeeks: number = nonTaperWeeks - Math.floor(nonTaperWeeks / planRecoverySpacing);
        // https://math.stackexchange.com/questions/2416120/reverse-continuous-compound-interest-formula-solve-for-r
        const growthRatePercent: number = Math.log(endDistance / startDistance) / growthWeeks;
    ...
    createActivitySpec = (userCriteria: UserSpec, distance: number, paceName: string = "warmup"): Activity => {
        ...
        const details: Run = {
            ...
            effort: {
                ...
                hr: {
                    min: roundTo((userCriteria.USER_MAX_HR / 100) * effortMatrix.lower_hrp, 5),
                    max: roundTo((userCriteria.USER_MAX_HR / 100) * effortMatrix.upper_hrp, 5)
                },
                hrp: {
    ...
    createLongDay = (date: Moment, userCriteria:UserSpec, distance: number = 5000, title: string = "Long", category: string = "long"): DayPlan => {
        return {
    ...
    createHitDay = (date: Moment, userCriteria: UserSpec, progress: ProgressType, distance: number = 5000): DayPlan => {
        return {
            day: this.createDay(date),
            primaryActivity: {
                type: "run",
    ...
    createBaseDay = (date: Moment, userCriteria: UserSpec, progress: ProgressType): DayPlan => {
        return {
            day: this.createDay(date),
            primaryActivity: {
                type: "run",
                category: "base",
                title: "Base run",
                details: this.createActivitySpec(userCriteria, this.nextBaseDistance(), "stamina")
            },
            extraActivities: [this.nextFlexRoutine(), this.nextCrossRoutine()]
        };
    }
    
    createEasyDay = (date: Moment, userCriteria: UserSpec, progress: ProgressType, distance: number = 3000): DayPlan => {
        return {
            day: this.createDay(date),
            primaryActivity: {
                type: "run",
    ...
    createWeekDay = (focus: string, date: Moment, userCriteria: UserSpec, progress: ProgressType): DayPlan => {
        const dayType: string = weekSchedule[focus][date.day()];
    
        switch (dayType) {
            case "race":
                return this.createLongDay(date, userCriteria, this.endDistance, "Race day", "race");
            case "long+":
                return this.createLongDay(date, userCriteria, calculateDistanceForWeek(this.currentWeek, 1.2, this.startDistance, this.growthRatePercent, this.endDistance, this.planStrategies) , "Longer run", "long+");
            case "pair_1":
                return this.createLongDay(date, userCriteria, calculateDistanceForWeek(this.currentWeek, 0.5, this.startDistance, this.growthRatePercent, this.endDistance, this.planStrategies) , "Pair #1", "1/2");
            case "pair_2":
                return this.createLongDay(date, userCriteria, calculateDistanceForWeek(this.currentWeek, 0.9, this.startDistance, this.growthRatePercent, this.endDistance, this.planStrategies) , "Pair #2", "2/2");
            case "long_r":
                return this.createLongDay(date, userCriteria, calculateDistanceForWeek(this.currentWeek, 0.75, this.startDistance, this.growthRatePercent, this.endDistance, this.planStrategies) , "Recovery", "long-");
            case "long_taper":
                return this.createLongDay(date, userCriteria, this.endDistance * 0.2, "Taper run", "taper");
            case "long":
                return this.createLongDay(date, userCriteria, calculateDistanceForWeek(this.currentWeek, 1, this.startDistance, this.growthRatePercent, this.endDistance, this.planStrategies) );
            case "hit":
            ...
            default: // Always rest where possible!
                return this.createRestDay(date, progress);
        };
    };
    

    createDayPlan = (weekStartDate: Moment, dayOfWeek: number, focus: string): DayPlan => {
    ...
    createWeekDays = (week: number =1, focus: string, weekStartDate: Moment): DayPlan[] => {
    ...
    createWeek = (w:number, focus:string): WeekPlan => {
        const newWeekStart: Moment = this.planStartDate.clone().add(w - 1, "weeks").startOf("day");
